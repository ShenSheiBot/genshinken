...
[图题] 1、メタレヴェル（存在）——meta level（存在）；2、オブジェクトレヴェル（存在者の集合＝世界）——object level（存在者的集合=世界）；3、現存在——此在（Dasein）。

![存在、存在者与现存在的二重襞示意图](/attachments/roof-archive/cv14594447/figure-5.jpg "=100%")

[图题] 1、実存論的構造（二レヴェルの媒介）——实存论的结构（两个level之间的媒介）；2、論理形式の産出——逻辑形式的生产；3、現存在（メタレヴェルへの入口）——此在（通往meta level的入口）。
