...
![T模式公式](attachments/roof-archive/cv2031971/04-schema-t.png)
...
![非限定摹状词公式](attachments/roof-archive/cv2031971/06-indefinite-description.png)

对于包含限定摹状词的语句（the...），将“这个具有性质P的事物存在”（指定了对象，因此是限定的）替换为“恰好有一个对象有性质P”：

![限定摹状词存在公式](attachments/roof-archive/cv2031971/07-definite-description-existence.png)

将“这个具有性质P的事物有性质Q”替换为“恰好有一个对象有性质P”与“任何对象只要有性质P就有性质Q”的合取（代表and的逻辑符号）。

![限定摹状词性质公式](attachments/roof-archive/cv2031971/08-definite-description-property.png)
...
![摹状词宽辖域与窄辖域公式](attachments/roof-archive/cv2031971/09-description-scope.png)
...
所谓存在预设就是说，对于一个包含个体常项$t$的语句$\varphi$，$\varphi$预设了$t$有所指，即不论$\varphi(t/x)$是否为真都能得到。这里我们与摹状词理论一样用量词与等词表示一个词项有指称，将$t$有指称记为，表示$t$在模型$M$中有定义。由此可知，虚构主义要求的引入的空词项的实质并不是在原有的词项中加入空词项，而是取消对于所有词项的存在预设，使得一阶语言中我们不再是先验的知道词项的指称是否存在。这是符合直觉的，因为一个名字的存在并不等于其指称的（现实）存在。
...
![二值原则预设推导](attachments/roof-archive/cv2031971/11-excluded-middle-derivation.png)
...
![存在预设与二值原则等值公式](attachments/roof-archive/cv2031971/12-presupposition-equivalence.png)
