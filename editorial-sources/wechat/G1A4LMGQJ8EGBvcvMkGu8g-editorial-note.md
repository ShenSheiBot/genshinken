...
- Canonical book mapping: 本条正文已并入《直面资本与帝国：重思京都学派哲学》唯一 book host /books/confronting-capital-and-empire 的章节锚点 `#chapter-02-labor-process-historical-time`；原微信 canonical URL 仍作为本章来源证据，独立公开稿已移除。

## 逐篇格式复核

- 逐项核对 `#js_content` 的 62 个非空正文单元、3 个标题与 1 张正文图片；正文、顺序和图片资产均完整。章节边界止于下一章 `#chapter-03-commodity-fetishism` 之前。
- 书籍 manifest 已正确记录原作者 William Haver、译者 Modus 与译校单字一兔；这些职责不在宿主文章 front matter 重复声明。
- 将上下文明确引出的 4 段马克思原文与 6 段西田原文恢复为块引文。原文中的 `/147/` 至 `/153/` 是引文页码标记，予以保留；书目信息和作者评注仍为普通段落。
- 派生 `metadata.json` 的简介混入其他篇目标题且被截断，不属于 `raw.html` 的页面事实，未用于公开正文或元数据。
