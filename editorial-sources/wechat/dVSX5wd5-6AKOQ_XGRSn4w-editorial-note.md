# BL 作为“性别扮演”的演变：从当代视角看其起源与发展：微信归档编辑记录
...
- 逐篇复核恢复了 raw 的六个正文标题、两组蓝色粗体编号要点、五段蓝色缩进引文、五幅书影／杂志图的 figure 题注，以及英译、编辑责任；其中两个编号问题曾被误升为章节标题，现已还原为强调列表。第 32 条原文注释中的微信文章题名继续保留为可点击链接。通用投稿邮箱与 QQ 仍按站点统一规则不重复放入正文。
- 逐图复核纠正第 4 幅图片的错误 alt：该图是永久保阳子《やおい小説論》，不是《玉三郎恋爱狂想曲》。另对 raw 中两处确定性损坏作最小修复：删除“少年爱”后的重复“混混”，将未闭合的“厌女情节：。”修为“厌女情结”。
- 公开标题省略微信题名前缀“【译文】”：译作身份已由“译”栏目及责任元数据表达，正文题名不重复平台式分类前缀。五幅图在 raw 中均为“图片后接图说”；公开稿使用站内约定的“`[图题]` 后接图片”源码顺序，渲染结果仍为图片与其 `figcaption` 成对显示，并非交换图文关系。
...
- block 0 的英文译者署名与 block 1 的原书出处/译者已写入 citation provenance；block 4 为 CC/平台尾部，按平台噪声删除。原文 33 条无调用尾注已在公开稿的“原文注释”中按普通项目列表逐条照录，不转成 GFM 脚注；正文、图片、链接和顺序无实质缺口。
- raw 直系子节点数、递归 DOM 节点数、可见文本节点数与 preservation 的语义块数分别服务于不同的计数契约，不能彼此冒充；完整性判断以 literal `#js_content`、逐块审读和当前 preservation 记录共同为准。append-only ledger 与旧 evidence 中的历史 body/Markdown hash 仅记录此前提取或此前公开稿，不覆盖当前 raw/body 与重建后的 canonical hash；本记录及 `manual-v2/087.json` 明确取代旧“clean”结论。
