...
- 逐节点报告：`.local-archive/wechat-full-format-audit/manual-v2/039.json`。raw 的142是132段、3个标题、3个figure、2个blockquote和2条分隔线的总块数，不是图片数；正文确有3张图。
- 报告 `039-05` 为路径解析误报。站点把 Markdown 的 `attachments/...` 映射到 `public/attachments/...`；公开目录中的 `002.jpg`、`003.jpg`、`004.jpg` 均存在，且字节哈希依次与 raw `body-001.jpg`、`body-002.jpg`、`body-003.jpg` 相同。
- 恢复译者注第6条，并将来源中的纯文本相关文章题名解析为稳定站内链接；恢复两个主体章节标题、两处分隔和“对后结构主义的批评”标题。
- 三个 raw figure 均转换为站内 `[图题]` figure 结构。首图 raw figcaption 明示“来自《放浪息子》，图文有关”；后两图保留人物身份，不把视觉描述冒充来源原有图题。
