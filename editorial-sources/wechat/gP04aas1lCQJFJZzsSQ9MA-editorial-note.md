...
- 正文无编号脚注调用或定义。raw 中 24 个非空 `margin-left: 32px` 段落是书目出处或译注，公开稿以 blockquote 还原其次级注释层级，不虚构脚注编号；两个开篇译注链接（《日本哲学入门：何谓“日本的哲学”》《日本哲学：在西欧中心主义与世界哲学之间》）以及正文各章的实质性互文链接均保留。源页未声明许可，故不添加许可字段。
- 本轮逐节点复核恢复了此前转换遗漏的试译记、封面图题及上述两个译注链接；`#js_content` 的两个居中小标题保留为章节标题。平台样式只转译为站内既有 Markdown 语义，不将编辑证据或归档说明写入公开正文。
- 系列证据：本条与 0010–0022 共 13 个组件均明确标注 Brill 2017 原书《Confronting Capital and Empire》，导论逐部分列出全书 12 章，公众号后续说明“全书译毕”。现已按原书顺序并入完整 canonical book 容器；独立稿仅作为迁移前证据留存。
- 未决：导论页码未在 raw 正文出现；DOI 按 Brill 书目记录，未臆补页码；未发现 B 站同文 canonical。

- Canonical book mapping: 本条正文已并入《直面资本与帝国：重思京都学派哲学》唯一 book host /books/confronting-capital-and-empire 的章节锚点 `#introduction`；原微信 canonical URL 仍作为本章来源证据，独立公开稿已移除。
