...
- 脚注闭合：正文调用与定义均为 `7` 条，逐条对应；未将缺失的“注释从省”补成新注。
- 系列证据：仅记录正文中明确出现的相关译札链接，不据标题相似建立 book/topic 容器；本条没有足以建立容器的前后篇声明；0009 的两条公众号推荐链接已按平台推广噪声删去。
- 未发现原文授权条款；页面“注释从省”作为源文事实公开保留，未补造缺失注释。raw 的可见「译注／疏」标题保留在七条闭合脚注之前；原表三个 `colspan=2` 层级行改用语义 HTML table 表达，避免 GFM 双栏重复同一层级标题。

## Preservation omission audit (archiveSequence 0009)

- block 0 中的试译身份、封面署名、「注释从省」与具名感谢均属文章特定信息，已在公开稿保留；仅删除通用学习声明与两条公众号推荐链接。旧空白 collateral 文件不作为本轮证据；当前 raw/body、专篇说明与 preservation contract 直接绑定本来源。primary canonical 本身无需进入只登记 duplicate/superseded 来源的 `source-dispositions.json`。
