...
export function translationEditionIsVisible(status, previewEnabled) {
  return status === "published" || (
    previewEnabled && (status === "draft" || status === "review" || status === "reviewed")
  );
}

export function translationPreviewEnabledForEnvironment(nodeEnv, explicitPreview) {
  return explicitPreview === "1" || nodeEnv !== "production";
}

export function translationPublicEnabledForEnvironment(explicitPublic) {
  return explicitPublic !== "0";
}
...
export function translationTitleBreaks(value, title, locale, source) {
