...
export function titleSegmentWidthEm(text: string): number {
  let em = 0;
...
export function longestTitleSegmentWidthEm(segments: string[]): number {
  return Math.max(1, ...segments.map(titleSegmentWidthEm));
}
...
export function isCompactTitleSegment(text: string): boolean {
  const em = titleSegmentWidthEm(text);
  return em > 0 && em <= 5.25;
}
