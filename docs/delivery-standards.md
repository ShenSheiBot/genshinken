...
- 已使用 `[图题]` 且图片替代文本明确标识为书封、书籍封面、期刊封面或杂志封面的语义书影，必须显式填写宽度。单本封面通常使用 `25` 或 `33`；多书并置、封面与内页合成等复合图版按构图使用 `50` 或 `66`。`verify:typography` 会拒绝未定宽的语义书影，防止其意外铺满正文栏。
...
允许的起始标记为 `[版式:资料目录] / [版式:时间轴] / [版式:阅读路径] / [版式:书单] / [版式:播客] / [版式:联络卡] / [版式:漫画]`，结束标记统一为 `[版式结束]`。一个标记只改变阅读组织，不授权删改正文、调整作品顺序或补写来源事实。普通长文、图片较多的文章和书目举例不因长度或数量自动进入特殊版式。新增版式先以一个真实页面验证需求，再扩展同一机制，不为单篇另造组件。
