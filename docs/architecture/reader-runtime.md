...
- 移动端页眉保留首页品牌图标、文章目录、繁简与阅读习惯入口；注释和文献共用一个焦点受控的底部 dialog。阅读习惯面板在任一断点打开时，唯一的三横线按钮从页眉工具组迁入面板标题行右侧并作为关闭入口；关闭后按钮回到页眉并恢复焦点，不另渲染 `×` 关闭按钮。该按钮在打开与关闭的整个过渡中始终保持在视口内；设置面板不得用会把标题行按钮带出屏幕的整体平移动画。
