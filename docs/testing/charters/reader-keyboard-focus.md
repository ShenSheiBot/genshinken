...
1. 打开阅读习惯 dialog，确认页眉中的阅读习惯按钮保持同一 DOM 节点和原位，dialog 标题行提供独立关闭按钮。
2. 确认初始焦点位于 dialog 容器；从容器按 `Shift+Tab` 到最后一个可聚焦操作，再按 `Tab` 循环至标题行关闭按钮和首个设置操作，全程不离开 dialog。
3. 用面板关闭按钮和 `Escape` 分别关闭，等待右侧抽屉退出动画结束，确认焦点恢复到页眉中始终连接的阅读习惯按钮。
