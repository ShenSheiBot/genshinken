...
1. 打开阅读习惯 dialog，确认全页仍只有一个“阅读习惯”按钮，按钮已从页眉迁入 dialog 标题行右侧，且没有独立 `×`。
2. 确认初始焦点位于 dialog 容器；从容器按 `Shift+Tab` 到最后一个可聚焦操作，再按 `Tab` 循环至标题行的“阅读习惯”按钮和首个设置操作，全程不离开 dialog。
3. 用迁入后的按钮和 `Escape` 分别关闭，等待退出动画结束并确认焦点恢复到重新挂载于页眉的“阅读习惯”按钮。
