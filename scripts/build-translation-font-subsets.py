...
def generated_outputs_are_current(files: list[Path]) -> bool:
    """Return whether the committed subset already matches the live corpus.

    This is the fast path used by normal dev, check, build, and deploy commands.
    The full verifier remains responsible for the detailed font contract.
    """
    ...
    if manifest.get("targetCodePointCount") != len(target_points):
        return False
    if manifest.get("targetCodePointSha256") != code_point_digest(target_points):
        return False
    ...
    if not isinstance(records, dict) or not records:
        return False
    expected_fonts = {record["family"]: record for record in (*FONTS, *FALLBACK_FONTS)}
    if set(records) != set(expected_fonts):
        return False
    for family, record in records.items():
        if not isinstance(record, dict):
            return False
        expected = expected_fonts[family]
        if record.get("sourceSha256") != expected["source_sha256"]:
            return False
        if record.get("sourceUrl") != expected["url"]:
            return False
        ...
        if not output.is_file() or digest_file(output) != expected_digest:
            return False
...
def corpus_inventory_is_current(files: list[Path]) -> bool:
    try:
        manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError):
        return False
    corpus_relative = [path.relative_to(ROOT).as_posix() for path in files]
    return (
        manifest.get("corpusFileCount") == len(files)
        and manifest.get("corpusFiles") == corpus_relative
        and manifest.get("corpusSha256") == corpus_fingerprint(files)
    )


def refresh_corpus_inventory(files: list[Path], lock: object) -> None:
    if getattr(lock, "closed", True):
        raise RuntimeError("Japanese font manifest refresh requires the font-build lock")
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    manifest["corpusFileCount"] = len(files)
    manifest["corpusFiles"] = [path.relative_to(ROOT).as_posix() for path in files]
    manifest["corpusSha256"] = corpus_fingerprint(files)
...
def main() -> None:
    ...
    if (
        args.if_stale
        and generated_outputs_are_current(files)
        and corpus_inventory_is_current(files)
    ):
        print("Japanese translation font subsets already match the live corpus")
        return
    lock = acquire_font_build_lock(ROOT)
    files = corpus_files()
    if args.if_stale and generated_outputs_are_current(files):
        if not corpus_inventory_is_current(files):
            refresh_corpus_inventory(files, lock)
            print("refreshed Japanese font corpus inventory without rebuilding font binaries")
        else:
            print("Japanese translation font subsets were synchronized by another process")
        lock.close()
        return
